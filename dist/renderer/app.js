...
var render_terrain = function (cell) {
    ...
    // Water conditions
    if (terrain === terrain_1.Terrain.WATER && elevation < 5) {
        return TerrainColors['WATER_DEEP'];
    }
    if (terrain === terrain_1.Terrain.WATER && elevation < 15) {
        return TerrainColors['WATER'];
    }
    if (terrain === terrain_1.Terrain.WATER) {
        return TerrainColors['WATER_SHALLOW'];
    }
    // Grass conditions
    if (terrain === terrain_1.Terrain.GRASS && elevation < 30) {
        var rand = Math.random();
        return (rand < .1) ?
            TerrainColors['GRASS_HILLS'] : // 20% hill grass
            TerrainColors['GRASS_LOWLAND']; // 80% lowland grass
    }
    if (terrain === terrain_1.Terrain.GRASS && elevation < 70) {
        var rand = Math.random();
        return (rand < .9) ?
            TerrainColors['GRASS_HILLS'] : // 80% hill grass
            TerrainColors['GRASS_LOWLAND']; // 20% lowland grass
    }
    if (terrain === terrain_1.Terrain.GRASS) {
        return TerrainColors['GRASS_MOUNTAIN'];
    }
    // Sand conditions
    if (terrain === terrain_1.Terrain.SAND) {
        return TerrainColors['SAND'];
    }
    // Rock conditions
    if (terrain === terrain_1.Terrain.ROCK && elevation < 98) {
        return TerrainColors['MOUNTAIN'];
    }
    if (terrain === terrain_1.Terrain.ROCK) {
